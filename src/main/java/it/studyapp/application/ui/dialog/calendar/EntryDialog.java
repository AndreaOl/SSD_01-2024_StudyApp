...
public class EntryDialog extends Dialog {
    ...
    private final DateTimePickerDialog fieldStart;
    private final DateTimePickerDialog fieldEnd;
    ...
    public EntryDialog(Entry entry, boolean newInstance) {
        ...
        fieldStart = new DateTimePickerDialog("Start");
        fieldEnd = new DateTimePickerDialog("End");
...
//        fieldEnd.setDateOnly(allDay);
        ...
        fieldRDays.setItemLabelGenerator(item -> item.getDisplayName(TextStyle.FULL, Locale.US));
        ...
        fieldAllDay.addValueChangeListener(event -> {
            ...
            if (resetPeriodOnAllDayChange && event.isFromClient()) {
                if (event.getValue()) {
                    ...
                    // reset the start to the same day with one day difference
        ...
        fieldStart.addValueChangeListener(event -> {
            ...
            if(event.getValue().isAfter(fieldEnd.getValue()) || event.getValue().isEqual(fieldEnd.getValue())) {
            	if(fieldAllDay.getValue())
                	fieldStart.setValue(fieldEnd.getValue().minusDays(1));
            	else
                	fieldStart.setValue(fieldEnd.getValue().minusHours(1));
            }
        });
        ...
        fieldEnd.addValueChangeListener(event -> {
            ...
            if(event.getValue().isBefore(fieldStart.getValue()) || event.getValue().isEqual(fieldStart.getValue())) {
            	if(fieldAllDay.getValue())
                	fieldEnd.setValue(fieldStart.getValue().plusDays(1));
            	else
                	fieldEnd.setValue(fieldStart.getValue().plusHours(1));
            }
        });
